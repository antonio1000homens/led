...
class InfrastructureContractTests(unittest.TestCase):
    ...
    def test_deployment_secrets_keep_todoist_out_of_github_and_lambda_environment(self):
        ...
        self.assertIn("scripts/load-ssm-secrets.sh", workflow)
        self.assertNotIn("secrets.BW_ACCESS_TOKEN", workflow)
        self.assertNotIn("vars.BW_NATIONAL_RAIL_TOKEN", workflow)
        self.assertNotIn("vars.BW_SECRET_ID_CF_DEPLOY_API_TOKEN", workflow)
    ...
    def test_todoist_feed_is_off_by_default_and_oauth_parameter_is_rotatable(self):
        ...
        self.assertIn('"${ROOT_DIR}/backend/todoist.py"', package)
        self.assertIn('"${ROOT_DIR}/shared/formatting.py"', package)
