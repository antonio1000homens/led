...
class RuntimeModeTests(unittest.TestCase):
    def test_up_enters_and_exits_diagnostic_mode(self):
        runtime = RuntimeMode()
        self.assertEqual(runtime.mode, "normal")
        self.assertTrue(runtime.handle("up"))
    ...
    def test_down_advances_rotation_only_in_normal_mode(self):
        runtime = RuntimeMode()
        rotation = FakeRotation()
        self.assertFalse(runtime.handle("down", rotation, 12))
        self.assertEqual(rotation.calls, [12])
        runtime.handle("up")
        runtime.handle("down", rotation, 13)
        self.assertEqual(rotation.calls, [12])
