...
class MissingKey(Exception): response={"Error":{"Code":"NoSuchKey"}}
class FakeS3:
...
class ConfigApiTests(unittest.TestCase):
    def test_defaults_include_both_parks(self):
        value=config_api.ConfigStore("bucket",FakeS3()).load()
        self.assertTrue(value["themeParks"]["thorpePark"]["enabled"]); self.assertTrue(value["themeParks"]["chessington"]["enabled"])
    def test_validation_rejects_zero_and_unknown_fields(self):
        value=copy.deepcopy(config_api.DEFAULT_DISPLAY_CONFIG); value["themeParks"]["chessington"]["iterations"]=0
    ...
    def test_store_round_trip(self):
        client=FakeS3(); store=config_api.ConfigStore("bucket",client); value=copy.deepcopy(config_api.DEFAULT_DISPLAY_CONFIG)
    ...
    def test_api_requires_bearer_token_and_persists_put(self):
        ...
        with patch.dict(os.environ,{"ADMIN_TOKEN":"secret-token"}), patch.object(config_api,"ConfigStore",return_value=fake_store):
            ...
            self.assertEqual(response["statusCode"],200); self.assertIn("state/config.json",client.objects)
