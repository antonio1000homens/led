...
aws s3 cp "${ROOT_DIR}/simulator/admin.html" "s3://${BUCKET}/admin" --content-type text/html --cache-control 'no-store' --only-show-errors
# Keep the old object briefly useful for bookmarks while /admin is canonical.
...
aws cloudfront create-invalidation --distribution-id "${DISTRIBUTION_ID}" --paths '/index.html' '/api/screens' '/admin' '/admin.html' '/api/control/v1/*' >/dev/null
